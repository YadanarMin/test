...
class AdminController extends Controller
{
    ...
    function ShowPDF($name){
        return response()->file('/var/www/html/iPD/public/PDF/'.$name);
    }
    
    public function FileDownload() {
        $file = '/var/www/html/iPD/public/PDF/宮原ss現場取り組み.pdf';
        $name = basename($file);
    ...
    function LoadDescriptionPage(){
        ...
        $pageName = $projectId = session('pageName');
        $info = $commonModel->GetPageDescriptionInfo($pageName); 
        
        return view('pageDescription')->with(["data"=>$info]);
    }
    function backup()
    {
     $forge = new ForgeModel();
     $projects = $forge->GetAllBackupProject();   
     return view('backup_setting')->with(["projects"=>$projects]);
    }

    function SetPageNameToSession(Request $request){
    ...
    function UploadImages(Request $request){
         if(!isset($request->message)){
              ...
              if($message == "save_info"){
                 $fileNames = json_decode($request->get("fileNames"));
                 $description1 = $request->get("desc1");
                 $description2 = $request->get("desc2");
                 $overWrite = $request->get("overWrite");
                 $commonModel = new CommonModel();
                 $pageName = session("pageName");
                 $result = $commonModel->SavePageDescriptionInfo($fileNames,$description1,$description2,$pageName,$overWrite);
                 return $result;
             }else if($message == "delete_image"){
                 $pageName = session("pageName");
                 $commonModel = new CommonModel();
                 $imagePath = $request->get("imagePath");
                 $result = $commonModel->DeleteImage($pageName,$imagePath);
                 return $result;
             }
...
/*:/
        ...
        echo"</pre>";return;*/
            foreach($version_db_info as $version){
                ...
                if(strpos($project_name,'新大阪宮原NKビル【大阪】') === false)continue;
                ...
                //skip if version_number is not equal to 110
                ...
                if($version_number != 107)continue; 
    ...
    public function FilterWindowAndDoorProperty($property,$element_name,$category_name,$viewName)
    {
        ...
        $dakimikomi = isset($sunpou["抱き見込み_"]) ? $sunpou["抱き見���み_"] : "";
        ...
        $hou_jouji_kaihou = isset($bouka["法_���時開放_"]) ? $bouka["法_常時開放_"] : "";
    ...
    function GetAutoSaveProjectUrns(){
        $query = "SELECT fp.name as project_name,fv.id,fv.forge_version_id,fv.version_number,fi.id as item_id,fi.name as item_name from tb_forge_version fv
                    LEFT JOIN  tb_forge_item  fi on fv.item_id = fi.id
                    LEFT JOIN tb_project fp on fi.project_id = fp.id
                    WHERE fp.auto_save_properties = 1 ORDER BY fi.id,fv.version_number";       
        $result = DB::select($query);
        return json_decode(json_encode($result),true);//change array object to array
    }
    ...
    function GetProjectLatedVersion(){
        $query = "SELECT  result.*, (select (forge_version_id) from tb_forge_version where id = result.id) as forge_version_id 
                    FROM 
                    (SELECT temp.item_id,temp.item_name,(temp.name) as project_name,(tv.id) as id,(tv.version_number)as version_number FROM 
                        (SELECT ti.id as item_id,tp.name,ti.name as item_name
                            FROM tb_project tp
