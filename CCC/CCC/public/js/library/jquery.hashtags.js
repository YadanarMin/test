...
(function($) {
	$.fn.hashtags = function() {
		...
		$(this).wrap('<div class="jqueryHashtags"><div class="highlighter"></div></div>').unwrap().before('<div class="highlighter"><p><br></p></div>').wrap('<div class="typehead"></div></div>');
		...
		$(this).on("keyup", function(evnt) {
			...
			if ($(elementAtCursor).text().includes("#")) {
				var y = $(elementAtCursor).offset().top;
				var x = $(elementAtCursor).offset().left;
				CreateHashTagPopup(x, y, elementAtCursor);
...
function CreateHashTagPopup(xPos, yPos, elementAtCursor) {
	var str = "";
	var active_element_text = $(elementAtCursor).text();
	var searchText = active_element_text.split('').pop(); //last item
	ActiveElement = $(elementAtCursor);
	$.each(hashtag_list, function(index, hashtag) {
...
function FindReplacePosition(caretPos, str, selectedStr) {
	var chars = str.split('');
	var char_pos = curPostion;
	console.log("caretpostion=======" + curPostion);
	do {

		console.log("char at=====");
		console.log(str);
		if (str.charAt(char_pos) == " " || str.charAt(char_pos) == "　") {
			break;
		}
		char_pos--;
	...
	//return caretPos
	var sub_str_1 = str.slice(0, char_pos);

	var sub_str_2 = str.slice(curPostion + 1);
	console.log(str + "\n" + sub_str_1 + selectedStr + sub_str_2);
	return sub_str_1 + selectedStr + sub_str_2;
...
function SetHashtag(ele, selectedTag) {
	//alert(ActiveElement.text());
	var caretPos = getCaretPosition(ActiveElement);
	var replacedStr = FindReplacePosition(caretPos, ActiveElement.text(), selectedTag);
	ActiveElement.text(replacedStr);
	MakeHighlight(inputDiv, inputDiv.html());
	$("#hashtag_popup").css({ display: "none" });
	return;
