...
$(document).ready(function() {
    ...
    $("#tblProjectList tbody").on("click", "tr td", function(e) {
        ...
        //             mention: {
        //                 allowedChars: /^[_a-zA-Z0-9ぁ-んァ-ン一-龯\(\)\（\）\.\・\-\ー]*$/,
    ...
    //alert($("#hidProjectCode").val());
    if ($("#hidProjectCode").val() != "") {

        $("#chkOpenNewTag").attr("checked", "checked");
        console.log("before text style");
        MakeTextStyleAndMention();

    }


    // $(".ql-editor").on("load", function(e) {
    //     $(this).hashtags();
    // });
    $(".tbl_report > .ql-editor").hashtags();



});

function MakeTextStyleAndMention() {

    // Text-editor
    var selected_report_type = $("#report_type").val();
    var editor_arr = [];
    // if (editor_arr.length > 0) {
    //     //alert(editor_arr.length);
    ...
    $.each(selected_report_type, function(k, rt) {
        ...
        $("#" + tblId + " tr").each(function(index) {
            if ($(this).find('td:eq(0)').find('.reports').hasClass('report-disabled') == false) { //is enable
                var container = $(this).find('td:eq(0)').find('.reports');
                var id = container.attr('id');
                //alert($("#" + id).find('.ql-editor').attr('class'));
                //$("#" + id).find('.ql-editor').hashtags();
                //var toolbars = $("#" + tblId).parent().find('.reports_header');
                console.log($(this).find('td:eq(0)').find('.ql-toolbar').length)
                if ($(this).find('td:eq(0)').find('.ql-toolbar').length) {
                    ...
                    var tool = $('#' + id).parent().parent().parent().prev();
                    var toolbarOptions = [
    ...
    // console.log("yaya=========");
    // console.log($(".ql-editor").attr('class'));
    $(".ql-editor").hashtags();
}
...
function CreateCurrentWeekReportHistory(data) {
    ...
    $.each(data, function(index, row) {
        ...
        var report_with_style = row["report_with_style"] == "" ? row["report"] : row["report_with_style"];
        tab_content += "<tr>";
        tab_content += "<td width='350px'>" + report_with_style + "</td>";
...
function SetReportData(tbl_id, reports) {
    ...
    $.each(reports, function(k, row) {
        ...
        if (row["saved_user_id"] != $("#hidLoginID").val()) {
            ...
            str += "<td>";
            if (row["isShow"] == 1)
                str += "<div class='reports report-disabled' id='" + id + "' contenteditable='false'>" + report_with_style + "</div>";
            else
                str += "<div class='reports report-disabled hide-report-row' id='" + id + "' contenteditable='false'>" + report_with_style + "</div>";
            ...
            canUpdate = true;
            str += "<tr id='" + row["saved_user_id"] + "'>";
            str += "<input type='hidden' name='hidPrvReport' id='hidPrvReport' value='" + report_with_style + "'>";
            if (reports.length == 1) {
                if (row["isShow"] == 1)
                    str += "<td><div class='reports default-size2' id='" + id + "' contenteditable='false'>" + report_with_style + "</div></td>";
                else
                    str += "<td><div class='reports default-size2 hide-report-row' id='" + id + "' contenteditable='false'>" + report_with_style + "</div></td>";
            }
            else {
                if (row["isShow"] == 1)
                    str += "<td><div class='reports' id='" + id + "' contenteditable='false'>" + report_with_style + "</div></td>";
                else
                    str += "<td><div class='reports hide-report-row' id='" + id + "' contenteditable='false'>" + report_with_style + "</div></td>";
            }
...
function GetSaveReportList(tbl_id) {
    ...
    $("#" + tbl_id + " tr").each(function(index) {
        ...
        if ($(this).find('td:eq(0)').find('.reports').hasClass('report-disabled') == false) { //is enable
            var hidReport = ($(this).find('input[type=hidden]').html());

            var report_str = ""

            $(this).find('td:eq(0)').find('.reports p').each(function() {
                if (report_str == "" && $(this).text() != "") {
                    report_str += $(this).text()
                }
                else {
                    if ($(this).text() != "")
                        report_str += "<br>" + $(this).text();
                    else
                        report_str += $(this).text();
                }
            })

            var report_with_style = ($(this).find('td:eq(0)').find('.reports .ql-editor').html());

            //var res = GetContentEditableText(r);
            //alert(hidReport + "\n" + res);
            var save_hashtag_list = [];
            // $(this).find('td:eq(0)').find('.highlighter span').each(function() {
            //     hashtag_list.push($(this).html());
            // });
            $(this).find('td:eq(0)').find('.reports .mention').each(function() {
                //alert($(this).text());
                save_hashtag_list.push($(this).text());
            })

            if ($(this).find('td:eq(0)').find('.reports').text() != "") {
                if (hidReport == undefined || hidReport != report_str) {
                    result.push({ "saved_user_id": saved_user_id, "report": report_str, "report_with_style": report_with_style, "hashtags": save_hashtag_list })
                }
...
function SaveData() {
    ...
    //alert(JSON.stringify(save_report_list));
