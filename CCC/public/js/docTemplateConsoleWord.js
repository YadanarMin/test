...
var combo_data = {"PJコード":"${pj_code}","工事基幹コード":"${kouji_kikan_code}","支店":"${shiten}", "確度":"${kakudo}",
                ...
                //Other than tb_allstore_info
                "プロジェクト基準点X":"${pjmgt_base_linex}","プロジェクト基準点Y":"${pjmgt_base_liney}","モデル作成(担当者)":"${pjmgt_model_tantou}","モデル作成(組織)":"${pjmgt_model_syozoku}",
                "サブコン空調(担当者)":"${pjmgt_sub_kuutyou_tantou}","サブコン空調(組織)":"${pjmgt_sub_kuutyou_syozoku}",
                "サブコン電気(担当者)":"${pjmgt_sub_denki_tantou}","サブコン電気(組織)":"${pjmgt_sub_denki_syozoku}",
                "FAB作図(担当者)":"${pjmgt_fab_sakuzu_tantou}","FAB作図(組織)":"${pjmgt_fab_sakuzu_syozoku}",
                "FAB製作(担当者)":"${pjmgt_fab_seisaku_tantou}","FAB製作(組織)":"${pjmgt_fab_seisaku_syozoku}",
                "使用ソフトウェア(建築設計)":"${pjmgt_sw_design}","使用ソフトウェア(構造設計)":"${pjmgt_sw_structure}",
                "使用ソフトウェア(設備空調)":"${pjmgt_sw_fac_k}","使用ソフトウェア(設備電気)":"${pjmgt_sw_fac_d}",
                "使用ソフトウェア(ワンモデル)":"${pjmgt_sw_model}","使用ソフトウェア(施工)":"${pjmgt_sw_sekou}",
                "使用ソフトウェア(生産設計)":"${pjmgt_sw_seisan_sekkei}","使用ソフトウェア(サブコン施工空調)":"${pjmgt_sw_sub_sekou_k}",
                "使用ソフトウェア(サブコン施工電気)":"${pjmgt_sw_sub_sekou_d}","使用ソフトウェア(FAB作図)":"${pjmgt_sw_fab_sakuzu}",
                "使用ソフトウェア(FAB製作)":"${pjmgt_sw_fab_seisaku}",
                "スケジュール備考(設計モデル作成)":"${pjmgt_sched_bikou_sekkei_model}","スケジュール備考(確認申請)":"${pjmgt_sched_bikou_kakunin_sinsei}",
                "スケジュール備考(積算model統合)":"${pjmgt_sched_bikou_model_tougou}","スケジュール備考(工事従事者決定)":"${pjmgt_sched_bikou_kouji_juujisya_kettei}",
                "スケジュール備考(現場工程決定)":"${pjmgt_sched_bikou_genba_koutei_kettei}","スケジュール備考(施工)":"${pjmgt_sched_bikou_sekou}",
                "スケジュール開始(引き渡し)":"${pjmgt_sched_hikiwatasi_start}","スケジュール完了(引き渡し)":"${pjmgt_sched_hikiwatasi_end}",
                "スケジュール備考(引き渡し)":"${pjmgt_sched_bikou_hikiwatasi}",
                "BIM実行計画書バージョン":"${pjmgt_version_bimactionplan}","現在の日付":"${current_date}"};
...
function TextSearch(){
    $('#txtSearch').keyup(function(){
        ...
        /*$("#tbUser tbody >tr").each(function(index) {
            ...
            var pj_name = $row.find("td:eq(5)").text();
            var ipd_staff = $row.find("td:eq(106)").text();
            //alert(name+"\n"+email);
            if(!pj_code.includes(textboxValue) && !pj_name.includes(textboxValue) && !ipd_staff.includes(textboxValue)){
        ...
        $("#tbCheckBox tbody >tr").each(function(index) {
            $row = $(this);
            // console.log($row);
            var tmpVal = $row.find('input[type=hidden]').val();
            var tmpArray = tmpVal.split('$');
            var pj_name = tmpArray[0];
            var pj_code = $row.find('input[type=checkbox]').val();
            var ipd_staff = tmpArray[1];
            // ipd_staff = ipd_staff == "" ? "empty" : ipd_staff;
            // console.log(pj_name+"\n"+pj_code+"\n"+ipd_staff);
            if(!pj_code.includes(textboxValue) && !pj_name.includes(textboxValue) && (ipd_staff == "" || !ipd_staff.includes(textboxValue))){
            //if(index != 0)//skip when body header row
                $row.hide();
...
function downloadTemplateFile(){
    ...
    window.location="/iPD/document/outputDefaultWordTemplate/"+templateFileName;
}
